...
How to read from properties file using JAVA:
 ...
 Why we are using static block?
    - Because static block runs before everything else and initialize everything in it.
    - Since we are opening files, and reading values, we want to make sure that everything (such browser type,
    url value) are ready for our source to use it.
 ...
 DRIVER UTILITY CLASS
    ...
    -Why are we learning this?
        ...
        - We are not able to pass the same driver instance when calling method from other classes

   SOLUTION:  Creating a driver utility that guaranties a single instance of the driver and does the initial
              setup (maximize, imp)
